...
UI_TEXT = {
    "English": {
        ...
        "autogenerated_info": (
            "No precomputed output file was found. The app generated the latest "
            "enriched dataset automatically."
        ),
    ...
    "Español": {
        ...
        "autogenerated_info": (
            "No se encontró un archivo de salida precomputado. La app generó "
            "automáticamente el dataset enriquecido más reciente."
        ),
    ...
    "Português": {
        ...
        "autogenerated_info": (
            "Nenhum arquivo de saída pré-computado foi encontrado. O app gerou "
            "automaticamente o dataset enriquecido mais recente."
        ),
        "kam_filter_label": "Filtrar por KAM",
...
def load_or_generate_enriched_data() -> tuple[pd.DataFrame, bool]:
    """Load precomputed CSV when available, otherwise run the local pipeline."""
    if OUTPUT_FILE.exists():
        return pd.read_csv(OUTPUT_FILE), False

    raw_df = load_restaurants_data()
    cleaned_df = clean_restaurants_data(raw_df)
    scored_df = score_restaurants(cleaned_df)
    enriched_df = add_recommendations(scored_df)
    export_enriched_restaurants(enriched_df)
    return pd.read_csv(OUTPUT_FILE), True


try:
    df, was_autogenerated = load_or_generate_enriched_data()
except Exception as exc:
    st.error(
        f"{t('missing_file_error', st.session_state['language'])}\n\nDetails: {exc}"
    )
    st.stop()
...
if was_autogenerated:
    st.info(t("autogenerated_info", language))
